# Hi, I'm Yalchin Hasanov

Junior Data Analyst with an economics background and practical experience in SQL, Excel, Power BI, and Python.

I recently completed and submitted all four weekly projects for the DevJoint Data Analytics Internship. During the internship, I worked on SQL analysis, Excel data analysis, Power BI dashboard development, and a marketing business case project.

## Skills and Tools

* **SQL:** Oracle SQL, SQLite, joins, aggregations, CTEs, and window functions
* **Excel:** Data cleaning, lookup formulas, PivotTables, and dashboards
* **Power BI:** Data modeling, Power Query, DAX, and interactive dashboards
* **Python:** Pandas, NumPy, Matplotlib, and Seaborn
* **Business Analysis:** KPI comparison, root cause analysis, business findings, and measurable recommendations

## Featured Work

### DevJoint Data Analytics Internship

A four-week project-based program covering SQLite, Excel data analysis, Power BI dashboard development, and marketing business case analysis.

[View the DevJoint Data Analytics Internship repository](https://github.com/Yalchin001/DevJoint-Data-Analytics-Internship)

### Data Analytics Portfolio

My portfolio includes selected SQL, Excel, Power BI, and Python projects with project descriptions and links.

[View my data analytics portfolio](https://yalchin-hasanov-portfolio.netlify.app/)

## Current Focus

* Improving my Power BI and Python skills
* Building practical data analytics projects
* Documenting my analysis clearly on GitHub
* Preparing for junior data analyst opportunities

## Connect With Me

* [LinkedIn](https://www.linkedin.com/in/yalchinhasanov/)
* [Portfolio Website](https://yalchin-hasanov-portfolio.netlify.app/)
